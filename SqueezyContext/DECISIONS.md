...
| Billers: hard delete only, no soft delete / deactivate | Activate/deactivate adds hidden state with no real benefit at household scale. Billers either exist or they don't. If a user wants to temporarily disable a biller, they delete it and re-enter it. Removes the active/inactive flag, the toggle control, and the "show inactive" view from the bill management UI entirely. |
| Unified application log | All logging across the entire product surface — FastAPI request layer, backend services, admin dashboard, frontend errors — feeds one log file (`backend/logs/squeezypay.log`). Presentation (filtering, views, tabs) is a UI concern and can be customized freely. The log itself is the single source of truth. Request traffic is added via FastAPI middleware with a `[REQUEST]` label so it is filterable without a separate log. |
