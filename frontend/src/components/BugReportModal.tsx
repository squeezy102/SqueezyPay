...
//     Creates the issue silently without opening a browser tab.
//  2. GitHub new-issue URL — pre-fills title + body; user clicks Submit in the browser.
...
function buildBody(
  ...
  const diagSection = diag
    ? [
        ...
        `- Alembic revision: ${diag.alembic_revision}`,
        `- Packaged: ${diag.frozen}`,
        `- Plaid configured: ${diag.plaid_configured}`,
        "",
        "### Table row counts",
        ...Object.entries(diag.table_counts).map(([t, c]) => `- ${t}: ${c}`),
        "",
        "### Settings",
        ...Object.entries(diag.settings).map(([k, v]) => `- ${k}: ${v}`),
        "",
        "### Last 50 log lines",
        "```",
        ...diag.log_tail.map(scrubLine),
        "```",
      ].join("\n")
    : "_Diagnostics unavailable_";
...
export default function BugReportModal({ currentTab, onClose }: Props) {
  ...
  const body = buildBody(step1, step2, step3, diag, currentTab, userAgent);
      ...
      <div className="relative w-full max-w-xl rounded-2xl bg-white dark:bg-slate-900 shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 pt-5 pb-3 border-b border-slate-200 dark:border-slate-700">
          <div className="flex items-center gap-2">
            <span className="text-2xl" role="img" aria-label="bug">🐛</span>
            <span className="font-semibold text-slate-800 dark:text-slate-100">Report a Bug</span>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          ...
          {step === "submit" && !submitResult && (
            <>
              <p className="text-sm text-slate-500 dark:text-slate-400">
                Choose how to submit. All methods send the same information.
              </p>
              ...
              <div className="rounded-xl border border-slate-200 dark:border-slate-700 p-4 space-y-3">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  ...
                  The token is used once, never stored. Don't have one?{" "}
                  <button
