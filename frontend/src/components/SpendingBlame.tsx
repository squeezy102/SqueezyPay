...
import { getPlaidBlame } from "../utils/api";
import type { BlameData } from "../types";
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center py-16 text-slate-400 dark:text-slate-500 gap-3">
      ...
      <p className="text-xs">Sync transactions from the Connected Banks section above</p>
    </div>
  );
}

export default function SpendingBlame() {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {!isLoading && !isError && totalSpending === 0 && <EmptyState />}
