...
function CredentialForm({ bill, existingId, initialUsername, initialPassword, onClose }: FormProps) {
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 px-4">
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xl w-full max-w-sm">
        ...
        {credQuery.isLoading ? (
          ...
          <CredentialForm
            bill={bill}
            existingId={credQuery.data?.id ?? null}
            initialUsername={credQuery.data?.username ?? ""}
            initialPassword={credQuery.data?.password ?? ""}
            onClose={onClose}
          />
        )}
      </div>
